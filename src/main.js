...
class Branch {
  ...
  grow() {
    ...
    requestAnimationFrame(function loop () {
      _this.growthAmount = Math.sin(2 * x);
      x += Math.PI/180;
      // _this.growthAmount += 0.1;

      if (_this.growthAmount <= 0.99999) { //  b.c. it might not make it *quite* there
        requestAnimationFrame(loop);
      } else {
        _this.grown = true;
        for (let i = 0; i < _this.children.length; i++) {
          setTimeout(() => {
            _this.children[i].grow();
          }, randRange(0, 1500));
        }
      }
    });
  ...
  draw(ctx) {
    ...
    let diff = [(this.x2 - this.x1) * this.growthAmount, 
                (this.y2 - this.y1) * this.growthAmount];
    ctx.lineTo(this.x1 + diff[0], this.y1 + diff[1]);
